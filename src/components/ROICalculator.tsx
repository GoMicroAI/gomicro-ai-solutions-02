...
const SliderInput = ({ label, value, onChange, min, max, step, unit, description }: { label: string; value: number; onChange: (v: number) => void; min: number; max: number; step: number; unit: string; description?: string }) => (
  <div style={{ marginBottom: 20 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
      ...
      <span style={{ ...STYLES.value, fontSize: 20 }}>{value}{unit}</span>
    </div>
    {description && <p style={STYLES.hint}>{description}</p>}
    <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} style={{ ...STYLES.slider, marginTop: 8 }} />
    <div style={{ display: 'flex', justifyContent: 'space-between', ...STYLES.hint, marginTop: 4 }}><span>{min}{unit}</span><span>{max}{unit}</span></div>
  </div>
);
...
const BoxInput = ({ label, value, onChange, unit, width, hint, step: inputStep = 'any' }: { label: string; value: number; onChange: (v: number) => void; unit?: string; width?: number; hint?: string; step?: string }) => (
  <div style={{ display: 'flex', alignItems: 'center', padding: '8px 0', gap: 12 }}>
    <div style={{ flex: 1 }}>
      <span style={STYLES.label}>{label}</span>
      {hint && <p style={{ ...STYLES.hint, marginTop: 1 }}>{hint}</p>}
    </div>
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
      <input type="number" value={value} step={inputStep} onChange={(e) => onChange(parseFloat(e.target.value) || 0)} style={{ ...STYLES.input, width: width || 80 }} />
...
const AiqcSlider = ({ label, value, onChange, min, max, step, unit, defaultVal, description }: { label: string; value: number; onChange: (v: number) => void; min: number; max: number; step: number; unit: string; defaultVal: number; description?: string }) => {
  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, background: 'rgba(74,222,128,0.04)', borderRadius: 10, border: '1px solid rgba(74,222,128,0.12)', minHeight: 120 }}>
      ...
      {description && <p style={{ ...STYLES.hint, color: 'rgba(74,222,128,0.5)', marginBottom: 6, minHeight: 16 }}>{description}</p>}
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} style={{ ...STYLES.slider, accentColor: BRAND.greenSave }} />
...
const SegmentSlider = ({ visual, recording, other, onChange }: { visual: number; recording: number; other: number; onChange: (s: Segments) => void }) => {
  ...
  return (
    <div style={{ marginTop: 12, padding: 12, paddingTop: 18, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <p style={{ fontSize: 11, color: BRAND.textMuted, marginBottom: 8, minHeight: 44 }}>% of QC time on visual inspection + record keeping</p>
...
const RejectionCauseSplitSlider = ({ qcErrorPct, handlingPct, onChange }: { qcErrorPct: number; handlingPct: number; onChange: (v: number) => void }) => {
  ...
  return (
    <div style={{ marginTop: 12, padding: 12, paddingTop: 18, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <p style={{ fontSize: 11, color: BRAND.textMuted, marginBottom: 8, minHeight: 44 }}>What percentage of rejections are due to QC errors vs handling issues? (refrigeration, late trucks, rough handling)</p>
      <div style={{ display: 'flex', fontSize: 10, fontWeight: 600, marginBottom: 4, userSelect: 'none', fontFamily: 'Arial, sans-serif' }}>
